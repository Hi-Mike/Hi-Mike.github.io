...
最近的学习逐渐的涉及到Android底层的一些东西，学习Android最好的东西也莫过于“Read The Fucking Source Code”。一步一步的来吧，先学习一下Android源码的目录结构。

在网上可以找到Android源码的地方有很多，除了官方站点， [grepcode.com](http://grepcode.com/) 、 [androidxref.com](http://androidxref.com/) 等地方都是不错的选择。无奈对 **grepcode** 的东西无法理解，还好 **XRef** 是按照标准目录结构来的。我主要就是拿来主义，将 [这里](http://www.cnblogs.com/yyangblog/archive/2011/03/02/1968880.html) 添加一些自己的理解。

需要知道的是Android系统仍在不断的发展之中，大版本之间新增加的东西不少。当然，基本的原理是不会变的。这里我看的是 [Android 4.4.4](http://androidxref.com/4.4.4_r1/xref/) 的源码：
...
|-- frameworks ---核心框架--Java及C++实现

|-- hardware ----部分厂家开源的硬解适配层HAL代码

|-- libcore -----核心库相关

|-- libnativehelper --系统支持库

|-- Makefile ----makefile文件

|-- ndk ----ndk相关代码，开发套件

|-- packages --应用程序包，内置应用就在这里
...
参考：[framework](http://blog.csdn.net/banketree/article/details/24718899)

```
frameworks 目录        （核心框架——java及C++语言）
.
|-- base                （基本内容）
|   |-- api              Android应用框架层类、属性、资源等的声明
|   |-- awt             （AWT库）
|   |-- camera          （摄像头服务程序库）
|   |-- cmds             系统启动时用到的重要命令commands：am、app_proce等
|   |-- core            （核心组件）
|   |-- data             Android下的资源（字体和声音等数据文件）
|   |-- docs            （文档）
...
|   |-- obex            （蓝牙传输库）
|   |-- opengl          （2D-3D加速库）
|   |-- packages         框架层的实现（设置、TTS、VPN程序）
|   |-- policy           框架实现（PhoneWindow等）
|   |-- sax             （XML解析器）
|   |-- services         各种服务程序（如AlarmManagerService等）
|   |-- telephony        电话通讯管理
|   |-- test-runner      测试工具相关
|   |-- tests            各种测试
|   |-- tools            aapt、aidl等工具
